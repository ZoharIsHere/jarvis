...
Replies are generated by Claude via a provider switch — `JARVIS_LLM=ollama`
points it at a local model instead, no code change. Local inference was
benchmarked on this hardware and isn't viable (a 2017 dual-core with no GPU
path floors at ~12.5s per reply, and the small models that fit under that
fabricate details); the switch exists so Apple Silicon later is a one-line
change. Numbers, quality comparison, and cloud pricing:
[`docs/BRAINS.md`](docs/BRAINS.md).

See [`JARVIS_MASTER.md`](JARVIS_MASTER.md) §9 for the full voice
architecture, and [`docs/PRIOR_ART.md`](docs/PRIOR_ART.md) for a survey of
comparable projects and reusable building blocks.
